...
st.markdown("""
    ...
    .stButton > button { width: 100%; background: rgba(25, 40, 79, 0.6) !important; border: 1px solid rgba(76, 175, 80, 0.5) !important; color: #4CAF50 !important; font-weight: 800 !important; font-size: 1.1rem !important; padding: 12px !important; border-radius: 8px !important; transition: all 0.3s ease !important; box-shadow: none !important; }
    ...
    [data-testid="stVerticalBlockBorderWrapper"] { background: rgba(25, 40, 79, 0.4) !important; backdrop-filter: blur(16px) !important; border: 1px solid rgba(255, 255, 255, 0.1) !important; border-radius: 16px !important; padding: 20px !important; transition: all 0.3s ease-in-out !important; }
    [data-testid="stVerticalBlockBorderWrapper"]:hover { border: 1px solid #4CAF50 !important; box-shadow: 0 0 25px 5px rgba(76, 175, 80, 0.5) !important; transform: translateY(-3px); }
...
if not st.session_state['logado']:
    ...
    with col_login:
        ...
        with st.form("login_form"):
            ...
            if st.form_submit_button("Conectar ao Hub"):
                credenciais_validas = {"prt_admin": "PRT2026", "arthur_okada": "PRT_Arthur2026", "guilherme_antunes": "PRT_Guilherme2026", "lucas_reis": "PRT_Lucas2026"}
                if usuario in credenciais_validas and credenciais_validas[usuario] == senha: 
                    st.session_state['logado'] = True; st.rerun() 
                elif usuario != "" or senha != "": st.error("🚫 Usuário ou senha incorretos. Tente novamente.")
else:
        ...
        with c_esq:
            with st.container(border=True):
                            ...
                            with st.spinner("Lendo e padronizando a base única..."):
                                try:
                                    ...
                                    if 'cod_individuo' in df_temp.columns and not df_temp.empty:
                                        ...
                                        st.session_state['df_res'] = df_temp # GUARDA O DATAFRAME COMPLETO
                                    else: st.warning("⚠️ Coluna de ID não encontrada.")
                                except Exception as e: st.error(f"Erro na leitura: {e}")

                with tab_multipla:
                    st.markdown("<p style='font-size: 0.85rem; color: #A0AABF;'>Anexe os 4 arquivos brutos para tratamento.</p>", unsafe_allow_html=True)
                        ...
                        if st.button("Unificar e Prever", use_container_width=True, key="btn_multi"):
                            with st.spinner("A tratar e unificar as 4 bases..."):
                                try:
                                    ...
                                    df_final = df1.merge(df2, on='cod_individuo', how='outer').merge(df3, on='cod_individuo', how='outer').merge(df4, on='cod_individuo', how='outer')
                                    ...
                                    if 'cod_individuo' in df_final.columns and not df_final.empty:
                                        ...
                                        st.session_state['df_res'] = df_final # GUARDA O DATAFRAME COMPLETO
                                    else: st.warning("⚠️ Coluna de ID não encontrada.")
                                except Exception as e: st.error(f"Erro ao unificar as bases: {e}")
                
                # ==========================================
                # EXIBIÇÃO DA TABELA E FICHA DO CLIENTE
                # ==========================================
                if 'df_res' in st.session_state:
                    ...
                    df_exibicao = df_res_atual.copy()
                    if busca: 
                        df_exibicao = df_exibicao[df_exibicao['cod_individuo'].astype(str).str.contains(busca, case=False, na=False)]
                        
                    # Mostrar a tabela apenas com ID e Probabilidade para não poluir
                    df_tabela = df_exibicao[['cod_individuo', 'Probabilidade (%)']].rename(columns={'cod_individuo': 'ID'})
                    st.dataframe(df_tabela.style.background_gradient(cmap='RdYlGn_r', subset=['Probabilidade (%)']), height=212, use_container_width=True, hide_index=True)
                    
                    # === NOVO: FICHA DO CLIENTE APARECE AO PESQUISAR ===
                    if busca and len(df_exibicao) > 0 and len(df_exibicao) <= 3:
                        st.markdown("<h3 style='color: #4CAF50; margin-top: 15px; font-size: 1.2rem;'>📋 Ficha do Cliente e Insights</h3>", unsafe_allow_html=True)
                        
                        for _, row in df_exibicao.iterrows():
                            # Função inteligente para caçar a coluna independentemente de como ela veio escrita no arquivo
                            ...
                            # Extração flexível das 6 variáveis solicitadas
                            tempo = pegar_valor(row, ['tempo', 'meses', 'dias', 'relacionamento'])
                            cobertura = pegar_valor(row, ['cobertura', 'tipo_cobertura', 'plano'])
                            qtd_prod = pegar_valor(row, ['num_produtos', 'qtd_produtos', 'produtos'])
                            nps = pegar_valor(row, ['nps', 'satisfacao', 'score'])
                            atraso = pegar_valor(row, ['atraso', 'dias_atraso', 'inadimplencia'])
                            idade = pegar_valor(row, ['idade'])
                            prob = row['Probabilidade (%)']
                            
                            # Geração de Insights Dinâmicos de Negócio
                            cor_risco = "#e74c3c" if prob >= 50 else ("#f39c12" if prob >= 20 else "#2ecc71")
                            icone_risco = "⚠️" if prob >= 50 else ("👀" if prob >= 20 else "✅")
                            
                            if prob >= 50:
                                txt_insight = "<b>Risco Crítico:</b> Altíssima propensão ao cancelamento. "
                                if atraso not in ["N/D", "0"]: txt_insight += f"O atraso no pagamento é um agravante claro. Priorize renegociação ativa."
                                elif nps not in ["N/D"] and str(nps) in ["0", "1", "2", "3", "4", "5", "6"]: txt_insight += f"O NPS Detrator ({nps}) denuncia insatisfação com a experiência. Acionar ouvidoria."
                                else: txt_insight += "Ação imediata necessária (benefícios ou upgrade)."
                            elif prob >= 20:
                                txt_insight = "<b>Atenção:</b> Risco moderado ou em transição. Campanhas preventivas de engajamento são recomendadas."
                            else:
                                txt_insight = "<b>Cliente Saudável:</b> Risco irrisório. Perfil ideal para ofertas de Cross-sell e aumento de portfólio."

                            st.markdown(f"""
                            <div style="background: rgba(25, 40, 79, 0.4); border-radius: 12px; border: 1px solid rgba(255,255,255,0.1); padding: 15px; margin-bottom: 15px; backdrop-filter: blur(10px);">
                                <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid rgba(255,255,255,0.1); padding-bottom: 10px; margin-bottom: 15px;">
                                    <h4 style="margin: 0; color: #FFF;">ID: <span style="color: #4CAF50;">{row['cod_individuo']}</span></h4>
                                    <span style="background: {cor_risco}; color: #FFF; padding: 4px 12px; border-radius: 20px; font-weight: bold; font-size: 0.9rem;">Risco Predito: {prob}%</span>
                                </div>
                                
                                <div style="display: grid; grid-template-columns: repeat(3, 1fr); gap: 15px; margin-bottom: 15px;">
                                    <div style="background: rgba(0,0,0,0.2); padding: 10px; border-radius: 8px;">
                                        <p style="margin: 0; font-size: 0.75rem; color: #A0AABF; text-transform: uppercase;">Tempo (Dias)</p>
                                ...
                                <div style="background: rgba(76, 175, 80, 0.1); border-left: 4px solid {cor_risco}; padding: 12px; border-radius: 0 8px 8px 0;">
                                    <p style="margin: 0; font-size: 0.9rem; color: #E0E0E0;">{icone_risco} {txt_insight}</p>
                                </div>
                            </div>
                            """, unsafe_allow_html=True)
    ...
    # ==========================================
        ...
        try:
            ...
            metricas = [{"col": col_m1, "titulo": "Acurácia Global", "valor": "87.4%", "cor": "#3498db", "desc": "Acertos totais do modelo"}, {"col": col_m2, "titulo": "Recall (Sensibilidade)", "valor": "84.2%", "cor": "#e74c3c", "desc": "Capacidade de reter quem ia sair"}, {"col": col_m3, "titulo": "F1-Score", "valor": "85.7%", "cor": "#9b59b6", "desc": "Equilíbrio precisão/recall"}, {"col": col_m4, "titulo": "AUC-ROC", "valor": "0.92", "cor": "#2ecc71", "desc": "Qualidade da separação de classes"}]
            for m in metricas:
                with m["col"]:
                    st.markdown(f"""<div style="background: rgba(25, 40, 79, 0.4); border-radius: 12px; border: 1px solid rgba(255,255,255,0.1); padding: 15px; text-align: center; backdrop-filter: blur(10px);"><p style="margin: 0; color: #A0AABF; font-size: 0.9rem;">{m['titulo']}</p><h2 style="margin: 5px 0; color: {m['cor']};">{m['valor']}</h2><p style="margin: 0; color: rgba(255,255,255,0.6); font-size: 0.8rem;">{m['desc']}</p></div>""", unsafe_allow_html=True)
            st.divider()
            ...
            with col_graf1:
                ...
                df_importancia = pd.DataFrame({"Variável": ["Tempo de Relacionamento", "Tipo de Cobertura (Básica)", "Qtd. Produtos Ativos", "NPS / Satisfação", "Atraso Pagamento (Dias)", "Idade"], "Impacto": [0.35, 0.22, 0.18, 0.12, 0.08, 0.05]}).sort_values(by="Impacto", ascending=True)
            ...
            with col_graf2:
                ...
                st.markdown("<p style='font-size: 0.9rem; color: #A0AABF;'>Comparativo de permanência vs evasão na base original.</p>", unsafe_allow_html=True)
                df_churn_dist = pd.DataFrame({"Status": ["Retidos (Ficaram)", "Churn (Saíram)"], "Quantidade": [87850, 12150]})
                fig_dist = go.Figure(data=[go.Pie(labels=df_churn_dist['Status'], values=df_churn_dist['Quantidade'], hole=.6, marker_colors=['#2ecc71', '#e74c3c'])])
                fig_dist.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', margin=dict(l=0, r=0, t=20, b=0), legend=dict(font=dict(color="#FFFFFF"), orientation="h", yanchor="bottom", y=-0.2, xanchor="center", x=0.5))
                fig_dist.add_annotation(text="12,15%<br>Taxa de Churn", x=0.5, y=0.5, font_size=16, font_color="#FFFFFF", showarrow=False)
            ...
            with col_mc:
                z = [[7500, 500], [300, 1700]]
                x = ['Previu Reter', 'Previu Churn']; y = ['Ficou (Real)', 'Saiu (Real)']
                fig_mc = px.imshow(z, text_auto=True, x=x, y=y, color_continuous_scale='Teal', aspect="auto")
                fig_mc.update_layout(paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', margin=dict(l=0, r=0, t=10, b=0), coloraxis_showscale=False, xaxis=dict(tickfont=dict(color='#FFFFFF')), yaxis=dict(tickfont=dict(color='#FFFFFF')))
                st.plotly_chart(fig_mc, use_container_width=True, config={'displayModeBar': False})
            with col_mc_texto:
                st.markdown("""<div style="background: rgba(25, 40, 79, 0.2); border-radius: 12px; padding: 20px; border-left: 4px solid #9b59b6;"><h4 style="margin-top: 0; color: #FFF;">Como interpretar este comportamento?</h4><p style="color: #A0AABF; line-height: 1.6;">A matriz ao lado ilustra o desempenho do modelo no grupo de testes. O maior foco estratégico da <b>PRT Seguradora</b> está no quadrante inferior direito (Verdadeiros Positivos) e inferior esquerdo (Falsos Negativos).</p><ul style="color: #A0AABF; line-height: 1.6;"><li>O modelo demonstra uma alta capacidade de identificar clientes que realmente vão sair (alto <i>Recall</i>).</li><li>Errar para o lado do "Falso Positivo" tem um custo operacional de retenção muito menor do que não detectar um cliente em risco real.</li></ul></div>""", unsafe_allow_html=True)
        except Exception as e: st.error(f"Erro ao renderizar gráficos: {e}")